                  ...
                  content: [
                    {
                      ...
                      text: styleRefDataUrl
                        ? (category === "babies" || category === "people" || category === "memorial" || category === "gifts"
                          ? `TASK: Recreate the reference scene from IMAGE 1 exactly, but use the PEOPLE from IMAGE 2 as the subjects.\n\nIMAGE 1 = REFERENCE SCENE TEMPLATE. Copy from it:\n- Exact same composition, camera angle, framing, crop\n- Exact same poses, body positions, gestures, expressions\n- Exact same costumes, wardrobe, accessories, props\n- Exact same background, setting, floor, wall\n- Exact same lighting, color grading, photographic style\n\nIMAGE 2 = SUBJECT SOURCE. CRITICAL: If IMAGE 2 contains MULTIPLE people (e.g. a mom AND a baby, or parent and child), you MUST preserve the IDENTITY (face, skin tone, hair color, hair style, distinctive features) of EVERY person in IMAGE 2 and map them onto the corresponding people in the reference scene:\n- The baby/child in the reference scene = the baby/child in IMAGE 2\n- The adult/mom in the reference scene = the adult/mom in IMAGE 2\nDo NOT invent or replace any person. Do NOT keep only one subject. Both faces must be clearly recognizable as the people in IMAGE 2. Do NOT copy IMAGE 2's clothing, background, or lighting — only the facial identities.\n\n${categoryContext}${perVariantPrompt ? `\n\n>>> SCENE INSTRUCTION: ${perVariantPrompt} <<<` : ""}\n\nCRITICAL OUTPUT RULES:\n1. Output ONLY the photo content at full-bleed. NO frame, NO mat/border, NO mockup chrome.\n2. Hyper-realistic photograph matching the reference scene 1:1.\n3. Every person from IMAGE 2 must appear with their real face preserved.\n4. Wardrobe/scene/props/lighting come from IMAGE 1 ONLY.`
